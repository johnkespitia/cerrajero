...
/**
 * Raised when the NC / ND emission cannot proceed because of fiscal
 * configuration gaps that are not the caller's fault: missing resolution,
 * unresolvable software PIN, etc. Controller should surface the structured
 * code while preserving HTTP semantics (502 for emitter failure, 409 for
 * resolution gaps).
 */
final class CreditDebitNoteUnavailableException extends CreditDebitNoteException
{
    ...
    public static function fiscalProfileMissing(): self
    {
        return new self(
            self::CODE_FISCAL_PROFILE_MISSING,
            'Parent document points to a CompanyFiscalProfile that no longer exists.'
        );
    }
